...
class Chef::Dialect::Python < Chef::Dialect
  register_dialect :recipe, '.py', 'text/x-python'
  #register_dialect :attributes, '.py', 'text/x-python'
  ...
  def compile_recipe(recipe, filename)
    py_run(filename, recipe.node) do |mod|
      each_resource do |res_name|
        create_method(res_name, mod) do |args, kwargs|
  ...
  def py_run(filename, node=nil, &block)
    LibPy.Py_InitializeEx(0)
    chef_mod = create_module('chef')
    create_method('debug', chef_mod) do |args|
  ...
  def create_method(name, mod=nil, &block)
    LibPy.module_eval do
      ...
      PyModule_AddObject(mod, name, fn) if mod
      fn
    end
  end
